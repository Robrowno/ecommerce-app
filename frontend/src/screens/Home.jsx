	...
	return (
		<div>
			<div className="relative w-full h-[400px] md:h-[500px] lg:h-[600px] flex items-center justify-center bg-gray-900">
				<img
					src="/hero-image.jpg"
					alt="Hero image of a laptop and mobile phone"
			...
			<div className="p-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					{laptops.map((laptop, index) => (
						<div key={index} className="border rounded-lg p-4 shadow-md">
							<img
								src={`${import.meta.env.VITE_API_URL}${laptop.image}`}
								alt={laptop.name}
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					{phones.map((phone, index) => (
						<div key={index} className="border rounded-lg p-4 shadow-md">
							<img
								src={`${import.meta.env.VITE_API_URL}${phone.image}`}
								alt={phone.name}
								className="w-full h-32 object-cover rounded"
							/>
			...
			<div className="w-full h-52 bg-gradient-to-b from-gray-800 to-[#121928] flex items-center justify-center px-6">
				<div className="text-center text-white">
					...
					<Link
						to="/products"
						className="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-lg shadow-lg transition duration-300"
					>
						Shop Now
					</Link>
				</div>
